...
// import { updateWorkOrder, updateWorkOrderLaravel } from '../../services/workOrder';
// import { fetchReasons } from '../../services/reason';
// import { fetchSBUs } from '../../services/sbu'; // ✅ NEW
...
//   const [nttnLinkIds, setNttnLinkIds] = useState([]);
//   const [reasons, setReasons] = useState([]);
//   const [sbus, setSbus] = useState([]); // ✅ NEW
//   const [ratesCache, setRatesCache] = useState(new Map());
//   const [currentNttnId, setCurrentNttnId] = useState(null);
//   const [currentLinkTypeId, setCurrentLinkTypeId] = useState(null);
//   const [currentRates, setCurrentRates] = useState([]);
...
//   const [isLoadingRates, setIsLoadingRates] = useState(false);
//   const [isLoadingReasons, setIsLoadingReasons] = useState(false);
//   const [isLoadingSbus, setIsLoadingSbus] = useState(false); // ✅ NEW
//   const [isSubmitting, setIsSubmitting] = useState(false);
//   const [lockedField, setLockedField] = useState(null);
//   const [isLoadingWorkOrders, setIsLoadingWorkOrders] = useState(false);
...
//   // ✅ NEW: Function to fetch SBUs
//   const fetchSbusList = async () => {
//     setIsLoadingSbus(true);
//     try {
//       console.log('📡 Fetching SBUs...');
//       const sbusRes = await fetchSBUs();
//       const sbusData = sbusRes?.data || sbusRes || [];

//       const mappedSbus = Array.isArray(sbusData)
//         ? sbusData.map((sbu) => ({
//             value: sbu.id,
//             label: sbu.name || sbu.sbu_name || `SBU ${sbu.id}`,
//           }))
//         : [];

//       console.log('✅ SBUs fetched:', mappedSbus);
//       setSbus(mappedSbus);
//     } catch (error) {
//       console.error('❌ Error fetching SBUs:', error);
//       setSbus([]);
//       showToast?.('Failed to load SBU list', 'warning');
//     } finally {
//       setIsLoadingSbus(false);
//     }
//   };
...
//             options={sbus}
//             onChange={(v) => formik.setFieldValue('sbu', v)}
//             searchable
...
import { fetchNTTNs } from '../../services/nttn';
import { fetchCategories } from '../../services/category';
import { fetchClientsCategoryWise, fetchClientsBySbu, fetchCategoriesBySBU } from '../../services/client';
import { fetchWorkOrderBeModification } from '../../services/workOrder';
import { fetchRatesByNttn, fetchRateByNttnAndBandwidth } from '../../services/rate';
import { fetchModificationTypes } from '../../services/modificationType';
import { createBWModification, updateBWModification } from '../../services/bwModification';
...
const BWModificationForm = ({ initialValues, isEditMode, onSubmit, onCancel, showToast }) => {
  ...
  const [isLoadingClientsBySbu, setIsLoadingClientsBySbu] = useState(false);
  ...
  const fetchSbusList = async () => {
    ...
    try {
      ...
      const mappedSbus = Array.isArray(sbusData)
        ...
        : [];
      console.log('SBU Test sample:', sbusData[0]);
      setSbus(mappedSbus);
    } catch (error) {
      setSbus([]);
      showToast?.('Failed to load SBU list', 'warning');
  ...
  fetchClientsCategoryWise(formik.values.client_category)
    .then((clientsRes) => {
      const clientsData = Array.isArray(clientsRes) ? clientsRes : clientsRes?.data || [];
       console.log('Client sample:', JSON.stringify(clientsData[0], null, 2)); // ← added
      setClients(clientsData);
  ...
  //     .then((clientsRes) => {
  //       const clientsData = Array.isArray(clientsRes) ? clientsRes : clientsRes?.data || [];
  //       console.log('Client sample=====:', clientsData[0]);
  //       setClients(clientsData);
  //     })
  //     .catch(() => setClients([]));
  // }, [formik.values.client_category]);
  ...
  // ✅ SBU → filter Category + fetch Clients by SBU
  useEffect(() => {
  ...
  // ✅ NEW: Client → SBU auto-select
  useEffect(() => {
    if (!formik.values.client) return;
    const selectedClient = clients.find(
      (c) => c.id === formik.values.client
    );
    if (selectedClient?.sbu_id) {
      formik.setFieldValue('sbu', selectedClient.sbu_id);
    }
      ...
      <form onSubmit={formik.handleSubmit} className="p-8 bg-gray-100 min-h-screen space-y-6">
        ...
        <FormSection title="Basic Info">
          ...
          <SelectField
            ...
            searchable
            // disabled={!formik.values.client_category}
            disabled={!formik.values.client_category || isLoadingClientsBySbu}
          />
        </FormSection>
