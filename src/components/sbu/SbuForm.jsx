// src/components/sbu/SbuForm.jsx
...
import { SbuSchema } from "../../validations/sbuValidation";
...
const FormSection = ({ title, children }) => (
  <fieldset className="col-span-full border-t border-gray-300 pt-6 mt-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-1 gap-x-6 gap-y-4 pt-4">{children}</div>
  </fieldset>
);

const SbuForm = ({ initialValues, isEditMode, onSubmit, onCancel, showToast }) => {
  const formik = useFormik({
    initialValues,
    validationSchema: SbuSchema,
        ...
        <div className="flex items-center space-x-3 mb-6 md:mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {isEditMode ? "Edit SBU" : "Add SBU"}
            </h1>
            <p className="text-gray-500">
              Fill in the details to {isEditMode ? "update" : "add a new"} Strategic Business Unit.
            </p>
          </div>
        </div>

        {/* Single section – same grid */}
        <FormSection title="SBU Information">
          <InputField
            name="sbu_name"
            label="SBU Name *"
