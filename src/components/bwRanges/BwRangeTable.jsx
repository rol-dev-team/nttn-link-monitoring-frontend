...
const BwRangeTable = ({ records, search, onEdit }) => {
  ...
  if (filtered.length === 0) {
    return <div className="text-center text-gray-500 mt-6">No records found.</div>;
  }
        ...
        <tbody>
          {filtered.map((item) => (
            <tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="border px-3 py-1">{moment(item.created_at).format('LLL')}</td>
              <td className="border px-3 py-1">{moment(item.updated_at).format('LLL')}</td>
