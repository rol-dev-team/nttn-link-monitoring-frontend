...
// import { fetchSurveysByClientLaravel, fetchSurveysDetailsByClient } from "../services/survey"; // Import survey functions
...
//       // Check if we should fetch survey data
//       let surveyData = null;
//       if (workOrder.client_id) {
//         try {
//           // Try to fetch survey data
//           const surveyResponse = await fetchSurveysByClientLaravel(workOrder.client_id);
//           const surveyDetailsResponse = await fetchSurveysDetailsByClient(workOrder.client_id);
          
//           // If surveys exist, use them
//           if (surveyResponse?.data?.length > 0) {
//             surveyData = surveyResponse.data[0]; // Use first survey
//           }
          
//           // Or use survey details if available
//           if (surveyDetailsResponse?.data?.length > 0) {
//             surveyData = surveyDetailsResponse.data[0];
//           }
//         } catch (surveyError) {
//           console.log('No survey data found or error fetching surveys:', surveyError);
//           // Keep existing work order data - don't override with empty values
//         }
//       }
...
//       const initialValues = {
//         ...workOrder,
//         // Use survey data if available, otherwise keep existing work order data
//         nttn_survey_id: surveyData ? (surveyData.survey_id || surveyData.id || '') : (workOrder.nttn_survey_id || ''),
//         nttn_lat: surveyData ? (surveyData.latitude || surveyData.lat || '') : (workOrder.nttn_lat || ''),
//         nttn_long: surveyData ? (surveyData.longitude || surveyData.long || '') : (workOrder.nttn_long || ''),
//         client_lat: surveyData ? (surveyData.client_latitude || surveyData.client_lat || '') : (workOrder.client_lat || ''),
//         client_long: surveyData ? (surveyData.client_longitude || surveyData.client_long || '') : (workOrder.client_long || ''),
