...
const CategoryForm = ({ initialValues, isEditMode, onSubmit, onCancel, showToast }) => {
  ...
  useEffect(() => {
    fetchSBUs()
      .then((res) => setSbuOptions(res.map((s) => ({ value: s.id, label: s.sbu_name }))))
      .catch((e) => showToast?.(e?.message || 'Could not load SBUs', 'error'));
  }, [showToast]);
      ...
      <form onSubmit={formik.handleSubmit} className="p-8 bg-gray-100 min-h-screen space-y-6">
        ...
        <fieldset className="col-span-full border-t border-gray-300 pt-6 mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 pt-4">
            ...
            {/* Category name */}
            <InputField name="cat_name" label="Category Name *" placeholder="Enter category name" />
          </div>
        </fieldset>
