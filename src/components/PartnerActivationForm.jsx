...
//     // New state for partner details and loading indicator
...
//     // Fetch client data from API (Runs once on mount)
//     useEffect(() => {
...
//                                         setPartnerDetails({
//                                             nttn_provider: response.data.nttn_name, // n.nttn_name
//                                             aggregator: response.data.aggregator_name, // a.aggregator_name
//                                             partner_name: response.data.client_name, // c.client_name
//                                             business_kam: response.data.kam_name, // k.kam_name
//                                             sbu: response.data.sbu_name, // s.sbu_name
//                                             purchased_capacity: response.data.request_capacity, // w.request_capacity
//                                         });
//                                     } else {
...
//                             fetchDetails();
//                         } else {
//                             // Clear details if no link ID is selected
//                             setPartnerDetails({});
//                         }
...
//                                     disabled={loading}
//                                     onChange={(e) => {
//                                         formik.setFieldValue("nttn_link_id", e.target.value);
//                                         setPartnerDetails({});
//                                     }}
//                                 />
//                                 {loading && (
...
//                                             label="NTTN Provider"
//                                             value={detailsLoading ? "Loading..." : partnerDetails.nttn_provider}
//                                         />
//                                         <PartnerDetailDisplayField
//                                             label="Aggregator"
//                                             value={detailsLoading ? "Loading..." : partnerDetails.aggregator}
//                                         />
//                                     </div>
//                                     <div className="space-y-4 px-5 border-r border-gray-100">
//                                         <PartnerDetailDisplayField
//                                             label="Partner Name"
//                                             value={detailsLoading ? "Loading..." : partnerDetails.partner_name}
//                                         />
//                                         <PartnerDetailDisplayField
//                                             label="Business KAM"
//                                             value={detailsLoading ? "Loading..." : partnerDetails.business_kam}
//                                         />
//                                     </div>
...
const ActivationPlanSchema = Yup.object().shape({
    ...
    work_order_id: Yup.string().required(
        "Partner Name / Work Order ID is required"
    ),
    nttn_vlan: Yup.string().max(10).required("NTTN VLAN is required"),
    int_peering_ip: Yup.string().max(50).required("IP is required"),
    ggc_peering_ip: Yup.string().max(50).required("IP is required"),
...
const getInitialValues = (initialData) => {
    if (initialData) {
        // Renamed nttn_link_id to work_order_id
        return {
...
// Interface Configuration Section (omitted for brevity, content unchanged)
const InterfaceConfigSection = ({ addToast }) => {
    // ... (Interface configuration logic as before)
    const { values, setFieldValue, setFieldTouched } = useFormikContext();
    const interfaceConfigs = values.interface_configs || [];
...
// Drop Device Section (omitted for brevity, content unchanged)
const DropDeviceSection = ({ addToast }) => {
    // ... (Drop device logic as before)
    const { values, setFieldValue, setFieldTouched } = useFormikContext();
    const dropDevices = values.drop_devices || [];
...
export default function PartnerActivationForm({
    ...
    return (
        <div className="w-full h-full p-4 lg:p-6">
            ...
            >
                {(formik) => {
                    // Use the clarified field name: work_order_id
                    const selectedLinkID = formik.values.work_order_id; 
                    
                    // Hook to Fetch Work Order Details when work_order_id changes
                    useEffect(() => {
                        if (selectedLinkID) {
                            const fetchDetails = async () => {
                                setDetailsLoading(true);
                    ...
                    return (
                        <Form className="grid grid-cols-1 md:grid-cols-3 gap-x-14 gap-y-6">
                            ...
                            <div className="">
                                <SelectField
                                    name="work_order_id" // Field name updated to work_order_id
                                    options={clientOptions}
                                    placeholder={loading ? "Loading clients..." : "Select Partner Name / Link ID"}
                                    className={"-mb-2"}
                                    disabled={loading}
                                    onChange={(e) => {
                                        formik.setFieldValue("work_order_id", e.target.value); // Field name updated
                                        setPartnerDetails({});
                                    }}
                                />
                                {loading && (
