...
const ShiftCapacityForm = ({ initialValues, isEditMode, onSubmit, onCancel, showToast }) => {
  ...
  const [sourceRateValue, setSourceRateValue] = useState(0);
  const [shiftingRateType, setShiftingRateType] = useState(null);
    ...
    getRatesBetweenShiftingBandwidth({
      ...
      .then((res) => {
        ...
        if (res?.data) {
          const rateTypeValue = res.data.rate_type ? parseInt(res.data.rate_type, 10) : null;
          setCurrentRateType(rateTypeValue);
          setShiftingRateType(rateTypeValue);
          setShiftingRateData(res?.data);
          const unitRate = parseFloat(res?.data?.rate) || 0;
          formik.setFieldValue('shifting_unit_cost', unitRate);
  ...
  /* ---------- Source: nttn_link_id selected → fetch rate and set capacity cost ---------- */
  useEffect(() => {
    if (!formik.values.nttn_link_id) return;
    const filtered = workOrderDetailsData?.find((item) => item.id === formik.values.nttn_link_id);
    if (filtered) {
      const requestCapacity = parseFloat(filtered.request_capacity) || 0;
      const linkTypeId = filtered.link_type_id || filtered.link_type || null;
      const rateValueFromAPI = parseFloat(filtered.rate) || 0;

      setPrintLinkTypeName(filtered.type_name);
      setSourceLinkTypeId(linkTypeId);
      setShiftingSourceRates(rateValueFromAPI);

      console.log('📌 Work Order Selected - capacity:', requestCapacity, '| link_type_id:', linkTypeId, '| rate:', rateValueFromAPI);

      formik.setFieldValue('capacity', requestCapacity);

      // ✅ Fetch rate_type from bandwidth API using work order's request_capacity
      const fetchRateTypeInfo = async () => {
        try {
          const res = await getRatesBetweenShiftingBandwidth({
            nttn_id: parseInt(formik.values.nttn_provider),
            link_type_id: linkTypeId,
            bandwidth: requestCapacity,
          });

          if (res?.data) {
            const rateTypeFromAPI = res.data.rate_type ? parseInt(res.data.rate_type, 10) : 2;
            const rateFromBandwidthAPI = parseFloat(res.data.rate) || 0;

            setSourceRateType(rateTypeFromAPI);
            setSourceRateValue(rateFromBandwidthAPI);

            console.log('🔍 Rate Type Fetch - rate_type:', rateTypeFromAPI, '| rate:', rateFromBandwidthAPI);

            // ✅ Calculate Capacity Cost based on actual rate_type
            let capacityCost = 0;
            if (rateTypeFromAPI === 1) {
              // Flat rate: use rate value directly (ignores capacity)
              capacityCost = rateFromBandwidthAPI;
              console.log('✅ Rate Type 1 (Flat): Capacity Cost =', rateFromBandwidthAPI);
            } else {
              // rate_type 2: multiply rate by capacity
              capacityCost = rateFromBandwidthAPI * requestCapacity;
              console.log('✅ Rate Type 2 (Variable): Capacity Cost =', rateFromBandwidthAPI, '×', requestCapacity, '=', capacityCost);
            }

            formik.setFieldValue('capacity_cost', parseFloat(capacityCost).toFixed(2) || 0);
          }
        } catch (error) {
          console.error('❌ Error fetching rate type:', error);
          // Fallback: assume rate_type 2
          const capacityCost = rateValueFromAPI * requestCapacity;
          formik.setFieldValue('capacity_cost', parseFloat(capacityCost).toFixed(2) || 0);
        }
      };
  ...
  }, [formik.values.nttn_link_id, formik.values.nttn_provider, workOrderDetailsData]);
  ...
  useEffect(() => {
    ...
    const woRequestCapacity = parseFloat(filtered.request_capacity) || 0;
    const woRate = parseFloat(filtered.rate) || 0;
    const woRateType = filtered.rate_type ? parseInt(filtered.rate_type, 10) : 2;

    setShiftingWorkOrderRate(woRate);
    setShiftingWorkOrderRequestCapacity(woRequestCapacity);
    setShiftingRateType(woRateType);

    formik.setFieldValue('target_request_capacity', woRequestCapacity);
    console.log('📊 Shifting Work Order selected — request_capacity:', woRequestCapacity, '| rate:', woRate, '| rate_type:', woRateType);
  ...
  useEffect(() => {
    ...
    if (upperBW > 0 && upperUnitPrice > 0) {
      const shiftingAmount = sourceRateType === 1 ? upperUnitPrice : upperBW * upperUnitPrice;
      formik.setFieldValue('shifting_amount_display', shiftingAmount.toFixed(2));
    } else {
      formik.setFieldValue('shifting_amount_display', '');
    }
  }, [formik.values.upper_shifting_bw, shiftingSourceRates, sourceRateType]);
  ...
  useEffect(() => {
    ...
    const fetchAfterShiftingRate = async () => {
      ...
      try {
        console.log('🔍 After Shifting Rate fetch — bandwidth:', afterShiftingCapacity, '| link_type_id:', linkTypeToUse, '| sourceRateType:', sourceRateType);
        const res = await getRatesBetweenShiftingBandwidth({
          nttn_id: parseInt(formik.values.nttn_provider),
          link_type_id: linkTypeToUse,
          bandwidth: afterShiftingCapacity,
        ...
        if (res?.data && res.data.rate) {
          ...
          console.log('🔍 API Response rate_type:', rateTypeValue, '| rate:', fetchedUnitRate, '| afterShiftingCapacity:', afterShiftingCapacity);

          // ✅ Handle rate_type: calculate total_shifting_cost accordingly
          let totalShiftingCost = 0;
          if (rateTypeValue === 1) {
            // Flat rate: use rate value directly (ignores capacity)
            totalShiftingCost = fetchedUnitRate;
            console.log('✅ Rate Type 1 (Flat): Total Shifting Cost =', fetchedUnitRate);
          } else {
            // rate_type 2: multiply by capacity
            totalShiftingCost = afterShiftingCapacity * fetchedUnitRate;
            console.log('✅ Rate Type 2 (Variable): Total Shifting Cost =', afterShiftingCapacity, '×', fetchedUnitRate, '=', totalShiftingCost);
          }
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-3 items-end">
          ...
          <div>
            ...
            {formik.values.upper_shifting_bw && shiftingSourceRates ? (
              <p className="text-xs text-gray-400 mt-0.5">
                {sourceRateType === 1
                  ? `Flat rate = ${formik.values.shifting_amount_display}`
                  : `${formik.values.upper_shifting_bw} × ${shiftingSourceRates} (Unit Price) = ${formik.values.shifting_amount_display}`}
              </p>
            ) : formik.values.upper_shifting_bw && !shiftingSourceRates ? (
              <p className="text-xs text-orange-400 mt-0.5">Select a Work Order Link ID first to get Unit Price</p>
            ) : null}
          ...
          <div className="col-span-full grid grid-cols-1 md:grid-cols-7 gap-x-6 gap-y-4">
            ...
            <div>
              ...
              {shiftingWorkOrderRequestCapacity && shiftingWorkOrderRate ? (
                <p className="text-xs text-gray-400 mt-0.5">
                  {shiftingRateType === 1
                    ? `Flat rate = ${shiftingWorkOrderRate}`
                    : `${shiftingWorkOrderRequestCapacity} × ${shiftingWorkOrderRate} = ${(shiftingWorkOrderRequestCapacity * shiftingWorkOrderRate).toFixed(2)}`}
                </p>
              ) : (
                <p className="text-xs text-gray-400 mt-0.5">
                  {shiftingRateType === 1 ? 'Flat rate' : 'request_capacity × rate'}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                value={(() => {
                  ...
                  const adj = parseFloat(adjustedAmount);
                  if (!isNaN(adj) && adjustedAmount !== '' && rate > 0) {
                    if (shiftingRateType === 1) {
                      return (rate + adj).toFixed(2);
                    }
                    const rawAfter = rate * (capacity + shiftBW);
                    return (rawAfter + adj).toFixed(2);
                  }
                  if (shiftingRateType === 1) {
                    return shiftingWorkOrderRate;
                  }
                  return shiftingWorkOrderRate && formik.values.target_total_bw
                    ? (shiftingWorkOrderRate * parseFloat(formik.values.target_total_bw)).toFixed(2)
                    : '';
              ...
              {(() => {
                ...
                if (shiftingRateType === 1) {
                  return <p className="text-xs text-gray-400 mt-0.5">Flat rate: {shiftingWorkOrderRate}</p>;
                }
                return shiftingWorkOrderRate && formik.values.target_total_bw ? (
                  <p className="text-xs text-gray-400 mt-0.5">
            ...
            <div>
              ...
              {adjustedAmount !== '' && shiftingWorkOrderRate && formik.values.target_total_bw ? (() => {
                ...
                const adj = parseFloat(adjustedAmount) || 0;
                if (shiftingRateType === 1) {
                  const adjAfter = rate + adj;
                  return (
                    <p className="text-xs text-blue-500 mt-0.5">
                      {rate} + ({adjustedAmount}) = {adjAfter.toFixed(2)}
                    </p>
                  );
                }
                const rawAfter = rate * (capacity + shiftBW);
