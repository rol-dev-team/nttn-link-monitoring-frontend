...
// src/components/roles/RoleFormModal.jsx
// Removed PermissionMatrix import
...
function RoleFormModal({ isOpen, onClose, role, addToast }) {
  const isEditing = !!role;
  const api = useApi();
  const queryClient = useQueryClient();
  ...
  const saveMutation = useMutation({
    mutationFn: (values) => {
      // Note: Permissions field is removed from values here, but if role object contained 
      // permissions, we ensure we only send the required metadata on update.
  ...
  return (
    <dialog className="modal modal-open">
      <div className="modal-box max-w-lg w-full"> {/* Reduced modal size */}
        <h3 className="font-bold text-lg mb-4">
          {isEditing ? `Edit Role Name: ${role.name}` : "Create New Role"}
        </h3>
        <Formik
          initialValues={initialValues}
          validationSchema={RoleSchema}
