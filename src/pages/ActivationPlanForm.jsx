...
const MOCK_VALID_VALUES = {
    ...
    drop_devices: [
        { device_ip: "10.1.1.1", usage_vlan: "201", connected_port: "G0/1/1" },
    ...
    interface_configs: [
        { interface_name: "Eth0/1", status: "active", note: "Primary link" },
        ...
        InterfaceConfigSchema.validate(newEntry, { abortEarly: false })
            ...
            .catch((validationErrors) => {
                validationErrors.inner.forEach((err) => {
                    ...
                    if (err.path === "status")
                        setFieldTouched("new_status", true);
                    if (err.path === "note") setFieldTouched("new_note", true);
                });
                addToast(
                    "Please fill in all required fields for the new interface.",
                    "error"
                );
            });
        ...
        async (values, { setSubmitting, resetForm }) => {
            ...
            try {
                ...
                const {
                    ...
                    new_interface_name,
                    new_status,
                    new_note, // 🔑 NEW temporary fields
                    ...submitValues
                } = values;

                // Log the clean data intended for the backend
                console.log(
                    "Submitting Activation Plan (Clean JSON):",
        ...
        <div className="w-full h-full p-6 lg:p-8 mt-10">
            <header className="mb-10 pb-6 border-b border-gray-200">
                ...
                <p className="text-lg text-gray-600 mt-2">
                    Configure technical parameters and associated drop devices
                    for a partner.
                </p>
            </header>
                    ...
                    return (
                        <Form className="grid grid-cols-1 md:grid-cols-3 gap-x-14 gap-y-6">
                            ...
                            <div className="space-y-1">
                                ...
                                />
                                {formik.touched.nttn_link_id &&
                                    formik.errors.nttn_link_id && (
                            ...
                            <div className="space-y-6">
                                <FieldWrapper
                                    name="nttn_vlan"
                                    placeholder="NTTN VLAN (max 10 chars)"
                                    formik={formik}
                                />
                                <FieldWrapper
                                    name="int_vlan"
                                    placeholder="INT VLAN (max 10 chars)"
                                    formik={formik}
                                />
                                <FieldWrapper
                                    name="ggn_vlan"
                                    placeholder="GGN VLAN (max 10 chars)"
                                    formik={formik}
                                />
                                <FieldWrapper
                                    name="fna_vlan"
                                    placeholder="FNA VLAN (max 10 chars)"
                                    formik={formik}
                                />
                                <FieldWrapper
                                    name="bdix_vlan"
                                    placeholder="BDIX VLAN (max 10 chars)"
                                    formik={formik}
                                />
                                <FieldWrapper
                                    name="mcdn_vlan"
                                    placeholder="MCDN VLAN (max 10 chars)"
                                    formik={formik}
                                />
                            </div>
