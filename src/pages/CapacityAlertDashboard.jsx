...
export default function CapacityAlertDashboard() {
  ...
  const filteredAlerts = useMemo(() => {
    ...
    return alerts.filter((row) => {
      ...
      if (filters.nas_ip_scope && row.activation_plan?.nas_ip !== filters.nas_ip_scope) {
        matches = false;
      }

      // Max Threshold Range Filter
      const maxMin = filters.max_threshold_mbps_min;
      if (maxMin !== null && maxMin !== '' && row.max_threshold_mbps < Number(maxMin)) {
        matches = false;
      }
      const maxMax = filters.max_threshold_mbps_max;
      if (maxMax !== null && maxMax !== '' && row.max_threshold_mbps > Number(maxMax)) {
        matches = false;
      }

      // Min Threshold Range Filter
      const minMin = filters.min_threshold_mbps_min;
      if (minMin !== null && minMin !== '' && row.min_threshold_mbps < Number(minMin)) {
        matches = false;
      }
      const minMax = filters.min_threshold_mbps_max;
      if (minMax !== null && minMax !== '' && row.min_threshold_mbps > Number(minMax)) {
        matches = false;
      }
  ...
  const handleEdit = async (alert) => {
    try {
      ...
      if (response.status) {
        ...
        throw new Error(response.message || 'Failed to fetch alert details');
      }
    } catch (err) {
  ...
  const handleDelete = async (alertId) => {
    if (!window.confirm('Are you sure you want to delete this capacity alert configuration?'))
      return;
      ...
      <header className="flex justify-between items-center mb-10 pb-6 border-b border-gray-200">
        <div>
          ...
          <p className="text-sm text-gray-500">
            View and manage maximum and minimum capacity alert thresholds.
          </p>
        </div>
        ...
        <DataTable
          ...
          filterComponent={
            <CapacityAlertFilterDrawer
              ...
              allNasIps={nasIps.map((ip) => ip.nas_ip).filter(Boolean)}
