// src/pages/link-type/LinkType.jsx
...
import LinkTypeForm from "../../components/link-type/LinkTypeForm";
import { createLinkType, fetchLinkTypes, updateLinkType, deleteLinkType } from "../../services/linkType";


const defaultInitialValues = { type_name: "" };
...
const LinkType = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    setError(null);
    try {
      const raw = await fetchLinkTypes();
      setRecords(raw.data);
    } catch (e) {
      const msg = e?.response?.data?.message || "Failed to load link types";
      setError(msg);
      pushToast(msg, "error");
    } finally {
      setLoading(false);
  ...
  const openEdit = (item) =>
    setFormState({
      ...
      isEditMode: true,
      editingId: item.id,
      initialValues: { type_name: item.type_name },
    });
  ...
  const handleDelete = async (id) => {
      ...
      try {
        const response = await deleteLinkType(id);
        if (response.success) {
  ...
  const handleSubmit = async (values) => {
    try {
      if (formState.isEditMode) {
        await updateLinkType(formState.editingId, values);
        pushToast("Updated successfully!", "success");
      } else {
        await createLinkType(values);
  ...
  const columns = useMemo(
    () => [
      ...
      { key: "type_name", header: "Link Type Name", isSortable: true },
      {
        key: "actions",
        header: "Action",
        render: (_, row) => (
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      <div className="flex justify-between items-center pb-16">
        <div>
          <h1 className="text-2xl font-bold">Link Type List</h1>
          <p className="text-gray-500">View and manage Link Types.</p>
        </div>
        <div className="flex items-center gap-4">
          <ExportButton
            data={records}
            columns={columns}
            fileName="link_type_list"
            intent="primary"
