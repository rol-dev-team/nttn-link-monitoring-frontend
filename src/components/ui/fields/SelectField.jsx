...
export default function SelectField({
  ...
  const formik = useFormikContext ? useFormikContext() : null;
  ...
  // ✅ FIX: Add flag to ignore focus event after selection/closing
  ...
  useEffect(() => {
    const onOutside = (e) => {
      ...
      // We check if the click target is outside the root container
  ...
  // FIX: Sync query with displayLabel on controlled value change
  useEffect(() => {
    if (!searchable) {
      setQuery(displayLabel);
    }
  ...
  // Keep query in sync for single select
  useEffect(() => {
    if (!multiple) {
      setQuery(displayLabel);
    }
    ...
    (val) => {
      if (name) {
        if (formik?.setFieldValue) {
          formik
            .setFieldValue(name, val, validateOnSelect)
            .then(() => formik.setFieldTouched(name, true, false))
            .catch(() => { });
        } else {
          helpers.setValue(val);
          if (validateOnSelect) helpers.setTouched(true, true);
        }
      }
      onChange?.(val);
    },
  ...
  const applySelection = (option) => {
    ...
    if (multiple) {
      ...
      // ✅ FIX 1: Set flag to ignore focus
      ignoreFocusRef.current = true;
      setOpen(false);
      setFocused(false);
      inputRef.current?.blur(); // Triggers the race condition
    }
  };
  ...
  const clearSelection = (e) => {
    ...
    setOpen(searchable);
    if (searchable) inputRef.current?.focus();
    else inputRef.current?.blur(); // ensure blur to trigger touched
  };
  ...
  const handleKeyDown = (e) => {
    ...
    switch (e.key) {
      ...
      case 'End':
        e.preventDefault();
        const lastIdx = [...filteredOptions]
          .map((o, i) => [o, i])
          .reverse()
          .find(([o]) => !o.disabled)?.[1];
        if (lastIdx !== undefined) setActiveIndex(lastIdx);
        break;
      case 'Enter':
    ...
    <div ref={rootRef} className={clsx('form-control mb-5', className)} {...rest}>
      <div className="relative">
        <div
          ...
          onClick={() => {
            if (disabled) return;
            // This is the trigger that opens/closes the dropdown.
            setFocused(true);
            setOpen((o) => !o);
          ...
          <div className="flex-1 flex flex-wrap items-center gap-1 pl-3">
            ...
            {searchable ? (
              <input
                ...
                onFocus={(e) => {
                  // ✅ FIX 2: Consume flag and prevent re-open after selection blur
                  if (ignoreFocusRef.current) {
                    ignoreFocusRef.current = false; // Reset the flag
                    return; // EXIT
                  }
                  e.stopPropagation();
                  setFocused(true);
              ...
              filteredOptions.map((opt, idx) => {
                ...
                return (
                  <li
                    ...
                    // ✅ FIX 3: Stop click propagation at the list item
                    onClick={(e) => {
                      e.stopPropagation(); // Prevents click from reaching parent div to re-toggle
                      !opt.disabled && applySelection(opt);
                    }}
                  >
                    {opt.label}
                  </li>
