...
const SurveyForm = ({ initialValues, isEditMode, onSubmit, onCancel, showToast }) => {
  ...
  useEffect(() => {
    const loadStaticData = async () => {
      try {
        ...
        setAggregatorOptions(aggregatorRes.map((item) => ({ value: item.id, label: item.aggregator_name })));
        setKamOptions(kamRes.map((item) => ({ value: item.id, label: item.kam_name })));
        setNttnOptions(nttnRes.map((item) => ({ value: item.id, label: item.nttn_name })));
        setCategoryOptions(catRes.map((item) => ({ value: item.id, label: item.cat_name })));
      } catch (err) {
        showToast(err?.response?.data?.message || "Failed to load static data for survey form!", "error");
      }
    };
    loadStaticData();
  }, [showToast]);
  ...
  useEffect(() => {
    const fetchClients = async () => {
      ...
      try {
        ...
        showToast(err?.response?.data?.message || "Failed to load clients for the selected category!", "error");
        setClientOptions([]);
      }
    };
    fetchClients();
  ...
  const handleClientChange = (selectedValue) => {
    ...
    const selectedClient = clientOptions.find((client) => client.value === selectedValue);
    if (selectedClient) {
  ...
  return (
    <div className="p-4">
      <div className="flex items-center space-x-2 mb-4 md:mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{isEditMode ? "Edit Survey" : "Add Survey"}</h1>
          <p className="text-gray-500">Fill in the details to {isEditMode ? "update" : "add a new"} survey record.</p>
        </div>
      </div>
