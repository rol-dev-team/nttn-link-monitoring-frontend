...
const SurveyFilterMenu = ({ records, onFilterChange, live = false, dynamicOptions }) => {
  ...
  const clientOptions = useMemo(() => {
    ...
    if (formik.values.sbu_id) {
      filteredRecords = filteredRecords.filter((record) => record.sbu_id === formik.values.sbu_id);
    }
  ...
  const activeFiltersCount = useMemo(() => {
    return Object.values(formik.values).filter((value) => value !== null && value !== '').length;
  }, [formik.values]);
    ...
    <>
      ...
      >
        <div className="flex-none flex items-center justify-between p-4 border-b border-gray-200">
          ...
          <Button onClick={handleCloseDrawer} variant="icon" size="sm" title="Close Filters">
