            ...
            <div className="sm:flex sm:items-start">
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div className="mt-2">
                  ...
                  <p className="text-sm font-semibold text-gray-700 mt-1">Link ID: {linkId}</p>
                </div>
              </div>
            </div>
          </div>
...
const getDownloadUtilizationColumns = (handleRunPause, handleResolveClick) => [
  ...
  { key: 'interface_port', header: 'Interface' },
  {
  ...
  },
  {
    ...
    header: 'Alert Config.',
    render: (v) => <div className="text-sm text-gray-700">{safeCell(v)}</div>,
  },
  // {
...
// Columns for ICMP Timeout Alerts
const getTimeoutAlertColumns = (handleRunPause, handleResolveClick) => [
  { key: 'client_name', header: 'Partner Name' },
  { key: 'nttn_work_order_id', header: 'NTTN Link ID' },
  ...
  //   key: 'day_count',
  //   header: 'Day Count',
  //   render: (v) => <span className="font-semibold text-gray-700 text-sm">{safeCell(v)} days</span>,
  // },
  {
    key: 'frequency_total',
    header: 'Frequency',
    render: (v, row) => (
      <span className="font-semibold text-gray-700 text-sm">
        {row.frequency_total} times in {row.day_count} days
      </span>
    ),
  },
...
// Columns for ICMP Latency Alerts
const getLatencyAlertColumns = (handleRunPause, handleResolveClick) => [
  { key: 'client_name', header: 'Partner Name' },
  { key: 'nttn_work_order_id', header: 'NTTN Link ID' },
  { key: 'last_latency_value', header: 'Last Latency' },
...
const PARTNER_COLUMNS = [
  { key: 'partner_name', header: 'Partner Name' },
  { key: 'contact_person', header: 'Business KAM' },
  {
    key: 'location',
    header: 'Address',
    render: (val) => (
...
export default function PartnerDashboard() {
  ...
  // New ICMP Alert States
  const [timeoutAlerts, setTimeoutAlerts] = useState([]);
  const [timeoutAlertCount, setTimeoutAlertCount] = useState(0);
  ...
  const fetchAllUtilizationAlerts = async () => {
    try {
      const [downloadResponse, uploadResponse, minDownloadResponse, minUploadResponse] =
        await Promise.all([
      ...
      if (minDownloadResponse.status && minDownloadResponse.data) {
        ...
        setMinDownloadUtilizationCount(
          minDownloadResponse.row_count || minDownloadResponse.data.length
        );
      }
  ...
  // Fetch ICMP Timeout and Latency alerts
  const fetchICMPAlerts = async () => {
    try {
      const [timeoutResponse, latencyResponse] = await Promise.all([
        getTimeoutAlerts().catch((error) => {
          console.error('Timeout Alerts API error:', error);
          return { summary: [] };
        }),
        getLatencyAlerts().catch((error) => {
          console.error('Latency Alerts API error:', error);
          return { summary: [] };
        }),
      ]);

      // Set timeout alerts
      if (timeoutResponse.summary) {
        setTimeoutAlerts(timeoutResponse.summary);
        setTimeoutAlertCount(timeoutResponse.summary.length);
      }

      // Set latency alerts
      if (latencyResponse.summary) {
        setLatencyAlerts(latencyResponse.summary);
        setLatencyAlertCount(latencyResponse.summary.length);
      }

      // Initialize alert status map for ICMP alerts
      const icmpAlertsMap = new Map();

      [...(timeoutResponse.summary || []), ...(latencyResponse.summary || [])].forEach((alert) => {
        if (alert.nttn_work_order_id) {
          icmpAlertsMap.set(alert.nttn_work_order_id, 'running');
        }
      });

      setAlertStatusMap((prev) => new Map([...prev, ...icmpAlertsMap]));
    } catch (error) {
      console.error('Error fetching ICMP alerts:', error);
    }
  };
  ...
  const fetchAllDashboardData = async () => {
    try {
      ...
      // Fetch all utilization alerts and ICMP alerts
      await Promise.all([fetchAllUtilizationAlerts(), fetchICMPAlerts()]);
    } catch (err) {
  ...
  const { totalMetrics } = useMemo(() => {
    ...
    const runningDownloadAlerts = downloadUtilizationAlerts.filter(
      (alert) => (alertStatusMap.get(alert.nttn_work_order_id) || alert.status) === 'running'
    ).length;

    const runningUploadAlerts = uploadUtilizationAlerts.filter(
      (alert) => (alertStatusMap.get(alert.nttn_work_order_id) || alert.status) === 'running'
    ).length;

    const runningMinDownloadAlerts = minDownloadUtilizationAlerts.filter(
      (alert) => (alertStatusMap.get(alert.nttn_work_order_id) || alert.status) === 'running'
    ).length;

    const runningMinUploadAlerts = minUploadUtilizationAlerts.filter(
      (alert) => (alertStatusMap.get(alert.nttn_work_order_id) || alert.status) === 'running'
    ).length;
    ...
    return {
      totalMetrics: {
        ...
        timeoutAlertCount: timeoutAlertCount,
        latencyAlertCount: latencyAlertCount,
  ...
  //   } else if (activeCard === 'timeout_alert') {
  //     baseAlertData = timeoutAlerts;
  //     titlePrefix = 'ICMP Timeout';
  //   } else if (activeCard === 'latency_alert') {
  //     baseAlertData = latencyAlerts;
  //     titlePrefix = 'ICMP Latency';
  //   } else if (activeCard === 'partners') {
  //     baseAlertData = partnerSummary;
  //     titlePrefix = 'Parnters Info';
  //   } else if (activeCard === 'aggregators') {
  //     baseAlertData = aggregatorSummary;
  //     titlePrefix = 'Aggregators Info';
  //   } else {
  ...
  //     const currentStatus =
  //       alertStatusMap.get(row.nttn_work_order_id || row.nttn_link_id) || row.status || 'running';
  ...
  //   setDynamicTableTitle(`${titlePrefix}`);
  //   return {
  ...
  const { dynamicTableData, baseAlertCount } = useMemo(() => {
    ...
    if (activeCard === 'max_download_alert') {
      ...
      // FIX: Use the correct partner data structure
      baseAlertData = partnerInfos.length > 0 ? partnerInfos : partnerSummary;
      titlePrefix = 'Partners Info';
    } else if (activeCard === 'aggregators') {
      baseAlertData = aggregatorSummary;
      titlePrefix = 'Aggregators Info';
    } else {
    ...
    // FIX: Only apply status filtering to alert types, not to partner/aggregator data
    let finalFilteredData = baseAlertData;

    // Only apply status filtering for alert types
    if (
      [
        'max_download_alert',
        'max_upload_alert',
      ...
      // Apply status-based filtering only for alerts
      finalFilteredData = dataWithCurrentStatus.filter((row) => {
        const status = row.current_alert_status;
        if (actionFilter === 'all') return true;
        if (actionFilter === 'running') return status === 'running';
        if (actionFilter === 'paused') return status === 'paused';
    ...
    aggregatorSummary,
    partnerSummary,
    partnerInfos, // ADD: Include partnerInfos in dependencies
  ]);
  /* --- Action Handlers --- */
  ...
  const handleStatCardClick = useCallback(
    (type) => {
      ...
      if (type === 'max_download_alert') {
        ...
        columns = getTimeoutAlertColumns(handleRunPause, handleResolveClick);
      } else if (type === 'latency_alert') {
        columns = getLatencyAlertColumns(handleRunPause, handleResolveClick);
    ...
    <div className="min-h-screen bg-gray-50 p-6 space-y-8">
          ...
          {initialLoading ? (
            ...
            <>
              <HealthStatCard
                ...
                valueClass={
                  activeCard === 'max_download_alert' ? 'text-red-700 underline' : 'text-red-600'
                }
              ...
              <HealthStatCard
                ...
                iconTextClass="text-red-600"
                valueClass={
                  activeCard === 'max_upload_alert' ? 'text-red-700 underline' : 'text-red-600'
                }
              ...
              <HealthStatCard
                ...
                valueClass={
                  activeCard === 'min_download_alert' ? 'text-blue-700 underline' : 'text-blue-600'
                }
              ...
              <HealthStatCard
                ...
                valueClass={
                  activeCard === 'min_upload_alert' ? 'text-cyan-700 underline' : 'text-cyan-600'
                }
                onClick={() => handleStatCardClick('min_upload_alert')}
              />

              {/* ICMP Timeout Alert Card */}
              <HealthStatCard
                title="ICMP Timeout Alerts"
                value={totalMetrics.timeoutAlertCount}
                subLabel="Network timeout issues detected"
                icon={WifiOff}
                iconBgClass="bg-orange-100"
                iconTextClass="text-orange-600"
                valueClass={
                  activeCard === 'timeout_alert' ? 'text-orange-700 underline' : 'text-orange-600'
                }
                onClick={() => handleStatCardClick('timeout_alert')}
              />

              {/* ICMP Latency Alert Card */}
              <HealthStatCard
                title="ICMP Latency Alerts"
                value={totalMetrics.latencyAlertCount}
                subLabel="Network latency issues detected"
                icon={Clock}
                iconBgClass="bg-orange-100"
                iconTextClass="text-orange-600"
                valueClass={
                  activeCard === 'latency_alert' ? 'text-orange-700 underline' : 'text-orange-600'
                }
                onClick={() => handleStatCardClick('latency_alert')}
              />
              ...
              <HealthStatCard
                ...
                valueClass={
                  activeCard === 'partners' ? 'text-indigo-700 underline' : 'text-gray-800'
                }
              ...
              <HealthStatCard
                ...
                valueClass={
                  activeCard === 'aggregators' ? 'text-yellow-700 underline' : 'text-gray-800'
                }
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
        <div className="flex justify-between items-center mb-4">
          ...
          {/* Filter Buttons */}
          {/* {["max_download_alert", "max_upload_alert", "min_download_alert", "min_upload_alert", "timeout_alert", "latency_alert"].includes(activeCard) &&
            baseAlertCount > 0 && (
