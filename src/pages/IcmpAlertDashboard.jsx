...
//       } else {
//         throw new Error(response.message || 'Failed to fetch ICMP alerts');
//       }
//     } catch (err) {
//       console.error(err);
...
//         setViewMode('form');
//       } else {
//         throw new Error(response.message || 'Failed to fetch ICMP alert details');
//       }
//     } catch (err) {
...
//             response = await updateIcmpAleartConfig(editingAlert.id, dataToSave);
//           } else {
...
//         <div>
//           <h1 className="text-3xl font-extrabold text-gray-900">ICMP Alert Configurations</h1>
//           <p className="text-sm text-gray-500">View and manage ICMP alert configurations.</p>
//         </div>
...
export default function IcmpAlertDashboard() {
  ...
  // Filter Options
  const NAS_IP_OPTIONS = useMemo(() => {
    const uniqueNasIps = Array.from(new Set(alerts.map(alert => alert.activation_plan?.nas_ip).filter(Boolean)));
    return uniqueNasIps.map(nasIp => ({
      value: nasIp,
      label: nasIp,
    }));
  }, [alerts]);
