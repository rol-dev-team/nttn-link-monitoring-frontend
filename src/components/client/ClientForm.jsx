...
//       rawObj.name ??
//       rawObj.label ??
//       (rawObj.value !== undefined ? String(rawObj.value) : `Option ${i}`);
...
//         console.log(
//           '[TELEMETRY] native submit event intercepted; defaultPrevented?',
//           e.defaultPrevented
//         );
//       }
//     };
...
//       setSbuOpts([]);
//       formik.setFieldValue('sbu_id', '', false);
//       return;
//     }
//     setLoadingSbu(true);
//     fetchSBUListCategoryWise(formik.values.cat_id)
//       .then((r) => {
//         if (active) setSbuOpts(normalizeList(r, 'id', 'sbu_name'));
//       })
//       .catch((e) => {
...
//       setDistricts([]);
//       formik.setFieldValue('district_id', '', false);
//       return;
//     }
//     setLoadingDis(true);
//     fetchDistrictDivisionWise(formik.values.division_id)
//       .then((r) => {
//         if (active) setDistricts(normalizeList(r, 'id', 'district_name'));
//       })
//       .catch((e) => {
...
//       setThanas([]);
//       formik.setFieldValue('thana_id', '', false);
//       return;
//     }
//     setLoadingTha(true);
//     fetchThanaDistrictWise(formik.values.district_id)
//       .then((r) => {
//         if (active) setThanas(normalizeList(r, 'id', 'thana_name'));
//       })
//       .catch((e) => {
...
//               Fill in the details to {isEditMode ? 'update' : 'add a new'} client record.
//             </p>
//           </div>
//         </div>
...
//             valueKey="id"
//             labelKey="cat_name"
//             disabled={loadingCat || loadingMasters}
//             required
//           />
//           <SelectField
//             name="sbu_id"
...
//             valueKey="id"
//             labelKey="sbu_name"
//             disabled={loadingSbu || !formik.values.cat_id}
//             required
//           />
//           <SelectField
//             name="division_id"
...
//             disabled={loadingDis || !formik.values.division_id}
//             required
//           />
//           <SelectField
//             name="thana_id"
...
//             valueKey="id"
//             labelKey="thana_name"
//             disabled={loadingTha || !formik.values.district_id}
//             required
//           />
...
import { fetchDivisions } from '../../services/utils';
import {
  fetchAllSBUs,
...
const ClientForm = ({ initialValues, isEditMode, onSubmit, onCancel, showToast }) => {
  ...
  /* -------------------- load master data (SBUs, Divisions) -------------------- */
  useEffect(() => {
    let active = true;
    ...
    const loadMasterData = async () => {
      ...
      try {
        const [sbusRes, divsRes] = await Promise.all([fetchAllSBUs(), fetchDivisions()]);

        if (!active) return;

        setSbuOpts(normalizeList(sbusRes, 'id', 'sbu_name'));
        setDivisions(normalizeList(divsRes, 'id', 'division_name'));
      } catch (e) {
      ...
      <form onSubmit={formik.handleSubmit} className="p-8 bg-gray-100 min-h-screen space-y-6">
        ...
        <FormSection title="Client Information">
          ...
          <SelectField
            ...
            valueKey="id"
            labelKey="cat_name"
            disabled={loadingCat || !formik.values.sbu_id}
            required
          />
