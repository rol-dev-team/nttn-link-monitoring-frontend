        ...
        <tbody>
          {records?.map((r, i) => (
            <tr key={i} className='hover:bg-gray-50'>
              ...
              <td className='border px-3 py-1'>{r.rate || "N/A"}</td>
...
//   // Function to calculate unit rate and total cost using API data
//   const calculateRates = (record) => {
//     // If unit_rate and total_cost are already provided by API, use them
//     if (record.unit_rate !== undefined && record.total_cost !== undefined) {
//       return {
//         unitRate: parseFloat(record.unit_rate).toFixed(2),
//         totalCost: parseFloat(record.total_cost).toFixed(2),
...
//             const unitRateFromAPI = r.unit_rate ? parseFloat(r.unit_rate).toFixed(2) : null;
//             const totalCostFromAPI = r.total_cost ? parseFloat(r.total_cost).toFixed(2) : null;
//             const calculatedTotalCost = r.calculated_total_cost ? parseFloat(r.calculated_total_cost).toFixed(2) : null;
            
//             // Priority: API total_cost > calculated_total_cost > calculate manually
//             let totalCost = totalCostFromAPI || calculatedTotalCost;
//             let unitRate = unitRateFromAPI;
            
//             // If no total cost from API, calculate it
//             if (!totalCost) {
...
//                 <td className='border px-3 py-1 font-medium text-green-600'>${unitRate || "0.00"}</td>
//                 <td className='border px-3 py-1 font-bold text-blue-600'>${totalCost || "0.00"}</td>
//                 <td className='border px-3 py-1'>{r.shift_capacity === "1" ? "Yes" : "No"}</td>
//                 <td className='border px-3 py-1'>{r.current_capacity || "N/A"}</td>
