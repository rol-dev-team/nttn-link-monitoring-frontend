...
import { useState } from 'react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { Plus, RefreshCw, ArrowLeft } from 'lucide-react'; // Added ArrowLeft
import { useApi } from '../hooks/useApi';
import { usePermissions } from '../hooks/usePermissions';
import Button from '../components/ui/Button';
import RolesTable from '../components/roles/RolesTable';
// import RoleFormModal from "../components/roles/RoleFormModal"; // REMOVED MODAL
import RoleFormPage from '../components/roles/RoleFormPage'; // ADDED DEDICATED PAGE
import DeleteRoleModal from '../components/roles/DeleteRoleModal';
import RolesTableSkeleton from '../components/roles/RolesTableSkeleton';
import ToastContainer from '../components/ui/ToastContainer';
import { useToast } from '../hooks/useToast';
...
function RolesPermissionsPage() {
  ...
  // Renamed state variables for the full-page view logic
  const [isFormView, setIsFormView] = useState(false); // Controls view switch
  const [selectedRole, setSelectedRole] = useState(null); // Role being edited/created
  const [roleToDelete, setRoleToDelete] = useState(null);
  ...
  // Mutation for deleting a role (unchanged)
  const deleteMutation = useMutation({
    mutationFn: (id) => api.deleteRole(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['roles'] });
  ...
  // Mutation for syncing permissions (unchanged)
  const syncPermissionsMutation = useMutation({
    mutationFn: () => api.syncPermissions(),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['permissions'] });
  ...
  const handleEditRole = (role) => {
    setSelectedRole(role);
    setIsFormView(true); // Switch to form view
  };
  ...
  // RENDER LOGIC: Switch between Table View and Form Page View
  if (isFormView && permissions) {
    return (
      <RoleFormPage
        role={selectedRole}
        permissions={permissions}
        onClose={handleBack} // 'onClose' now functions as 'onBackToTable'
        addToast={addToast}
      />
    );
  }
