...
export default function SelectField({
  ...
  /* * FIX: This useEffect ensures the input text is set to the selected label 
   * when component mounts/re-initializes (edit mode) or when the value/options change.
   */
  useEffect(() => {
    if (!multiple) {
      // Set query to the display label if there is a value
      setQuery(hasValue ? displayLabel : ''); 
    }
  }, [displayLabel, hasValue, multiple]);
  // END FIX

  /* ---------- outside click + reset search on outside-close ---------- */
  useEffect(() => {
    if (!open) return; // nothing to do while closed

    const onOutside = (e) => {
      if (rootRef.current && !rootRef.current.contains(e.target)) {
        setQuery(displayLabel); // Set query to selected label
        setInternalQuery('');
        setOpen(false);
        setFocused(false);
        if (name) helpers.setTouched(true);
    ...
    document.addEventListener('mousedown', onOutside);
    return () => document.removeEventListener('mousedown', onOutside);
  }, [open, helpers, name, multiple, displayLabel]); // Added displayLabel dependency
  ...
  const applySelection = (option) => {
    ...
    if (multiple) {
      ...
      /* ----- clear search & keep focus inside ----- */
      setQuery('');
      setInternalQuery('');
      inputRef.current?.focus();
      /* ----- DO NOT close dropdown for multi ----- */
    } else {
      /* ----- single select ----- */
      commitValue(option.value);
      setQuery(option.label); // <- input shows the choice
      
      /* ----- close & blur immediately ----- */
      setOpen(false); // <- close list
      setFocused(false);
      inputRef.current?.blur();   // <- removes the “still taking input” feel
    }
  };
  ...
  const clearSelection = (e) => {
    ...
    setQuery('');
    setInternalQuery(''); // Clear internal query too
    setOpen(searchable);
    if (searchable) inputRef.current?.focus();
  };
  ...
  const closeActions = () => {
    ...
    setFocused(false);
    if (name) helpers.setTouched(true);
    setQuery(displayLabel); // Set query to selected label on escape
    setInternalQuery('');
  };
  ...
  const handleKeyDown = (e) => {
    ...
    switch (e.key) {
      ...
      case 'Escape':
        if (open) {
          e.preventDefault();
          // Use the consolidated close actions
          closeActions();
        }
        break;
      case 'Backspace':
      ...
      <div className="relative">
        <div
          ...
          onClick={() => {
            ...
            /* --- NEW: wipe old search (set query to empty, list will open) --- */
            if (searchable && !multiple) {
              setQuery('');
              setInternalQuery('');
            }
          ...
          <div className="flex-1 flex flex-wrap items-center gap-1 pl-3 min-h-[2.5rem]">
            ...
            {(searchable || multiple) ? (
              <input
                ...
                value={query}
                placeholder={(!hasValue || (multiple && selectedKeySet.size === 0)) ? placeholder || displayLabel || '' : ''}
                onChange={(e) => {
                  setQuery(e.target.value);
                  if (!open) setOpen(true);
                  updateQuery(e.target.value);
                ...
                onFocus={(e) => {
                  ...
                  setFocused(true);
                  setOpen(true);
                  // Clear query on focus for single searchable fields
                  if (searchable && !multiple) {
                     setQuery('');
                     setInternalQuery('');
                  }
