...
const BWModificationTable = ({ records, search, onEdit }) => {
  ...
  // Helper function to display modification type
  ...
  const filtered = safeRecords.filter(
    (r) =>
      (r.nttn_link_id && r.nttn_link_id.toLowerCase().includes(search.toLowerCase())) ||
      (r.modification_type &&
        getModificationTypeDisplay(r.modification_type)
          .toLowerCase()
      ...
      <table className="min-w-full border border-gray-300 text-sm">
        ...
        <tbody>
          {filtered.map((item) => (
            <tr key={item.id} className="hover:bg-gray-50">
              <td className="border px-3 py-1">{item.nttn_provider || '-'}</td>
              {/* Updated modification type display */}
              <td className="border px-3 py-1">
                {getModificationTypeDisplay(item.modification_type)}
              </td>
              <td className="border px-3 py-1">{item.client_category_details?.cat_name ?? '-'}</td>
              <td className="border px-3 py-1">{item.client_details?.client_name ?? '-'}</td>
              <td className="border px-3 py-1">{item.nttn_link_id || '-'}</td>
              <td className="border px-3 py-1">{item.capacity || '-'}</td>
              <td className="border px-3 py-1">{item.capacity_cost || '-'}</td>
              <td className="border px-3 py-1">{item.shifting_bw || '-'}</td>
