...
//   // 💡 1. GET THE CORRECT DATA SLICE (Now defined first)
//   const currentPageSize = isBackendPagination ? pageSizeProp : pageSize;
//   const currentPage = isBackendPagination ? pageProp : page;

//   // Preliminary calculation of client-side safe page
//   const totalRowsPlaceholder = isBackendPagination ? totalRowsProp : 0;
//   const totalPagesPlaceholder = Math.max(1, Math.ceil(totalRowsPlaceholder / currentPageSize));
//   const pageSafePlaceholder = Math.min(Math.max(currentPage, 1), totalPagesPlaceholder);
...
//       // Slicing: Recalculate pageSafe based on actual filtered total for client-side
...
//     }
//   }, [data, query, inferredCols, sort, isBackendPagination, page, pageSize, totalRowsProp]); // Removed pageSafe from dep


//   // 💡 2. DEFINE FINAL PAGINATION CONTROLS (Now safe to use totalClientRows)
...
//   const pageSafe = Math.min(Math.max(currentPage, 1), totalPages); // The final, safe page number
...
//     // Uses the final pageSafe and currentPageSize/pageSize
//     return isBackendPagination
//       ? (pageSafe - 1) * currentPageSize + relativeIndex
//       : (pageSafe - 1) * pageSize + relativeIndex;
//   };
...
//       const rows = selectedIndices.map(absIndex => sortedData[absIndex]).filter(r => r);
//       onSelectionChange(rows);
//     }
//   }, [selected, sortedData, onSelectionChange]);
...
//             {/* Only show client-side filter info if not using backend pagination */}
